...
/**
 * Where a landmark should stand on a country: the point on the largest
 * landmass that is furthest from any coast. Unlike a centroid, it is always on
 * land and isn't pulled away by outlying islands.
 */

/** Twice the signed area of a ring. The sign gives its direction. */
...
/** Indices of the rings at least `minShare` the size of the largest one. */
...
/** Centre of a ring. Only a fallback. */
...
/** Grid size for each search pass, and how many times the search zooms in. */
const GRID = 24;
const REFINEMENTS = 4;

/** The point inside a ring furthest from its edges. Scans a grid, then zooms in on the best cell. */
function poleOfInaccessibility(ring: readonly Point[]): Point {
  let minX = Infinity;
  let minY = Infinity;
  let maxX = -Infinity;
  let maxY = -Infinity;
  ...
  for (let pass = 0; pass <= REFINEMENTS; pass++) {
    ...
    for (let i = 0; i <= GRID; i++) {
      for (let j = 0; j <= GRID; j++) {
        ...
        // Skip points outside the ring (cheaper than measuring distance).
    ...
    // Very thin land can miss every grid point; use its centre instead.
    if (bestDistance === -Infinity) return ringCentroid(ring);
    // Next pass: a small window around the best point.
    width = stepX * 3;
    height = stepY * 3;
...
/** The most inland point of the largest landmass, or null if there are no rings. */
