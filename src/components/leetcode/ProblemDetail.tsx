...
const DESCRIPTION_STYLES = [
  ...
  '[&_strong]:font-semibold [&_strong]:text-stone-900 dark:[&_strong]:text-stone-50',
  '[&_code]:font-mono [&_code]:text-[0.875em] [&_code]:bg-stone-100 dark:[&_code]:bg-stone-800 [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:rounded',
  '[&_pre]:my-4 [&_pre]:p-4 [&_pre]:bg-stone-100 dark:[&_pre]:bg-stone-900 [&_pre]:rounded-lg [&_pre]:font-mono [&_pre]:text-sm [&_pre]:overflow-x-auto',
  ...
  '[&_a]:underline [&_a]:text-stone-900 dark:[&_a]:text-stone-50',
  '[&_table]:my-4 [&_table]:text-sm [&_td]:border [&_td]:border-stone-300 dark:[&_td]:border-stone-700 [&_td]:px-3 [&_td]:py-1.5 [&_th]:border [&_th]:border-stone-300 dark:[&_th]:border-stone-700 [&_th]:px-3 [&_th]:py-1.5',
].join(' ');
  ...
  return (
    <article className="animate-[fadeInUp_0.5s_ease-out]">
      <header className="mb-10">
        <p className="font-mono text-sm text-stone-400 dark:text-stone-500 mb-2">#{problem.id}</p>
        <h1 className="text-4xl md:text-5xl font-heading font-semibold text-stone-900 dark:text-stone-50 mb-5 leading-[1.1] text-balance">
          {problem.title}
        </h1>
        <div className="flex flex-wrap items-center gap-2">
          ...
          {problem.topics.map((t) => (
            <span key={t} className="text-xs font-medium px-2.5 py-1 rounded-full border border-stone-200 dark:border-stone-800 bg-white dark:bg-stone-900 text-stone-600 dark:text-stone-400">
              {t}
            </span>
          ))}
        </div>
      ...
      <div className="lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)] lg:gap-12 lg:items-start">
        <section className="mb-12 lg:mb-0">
          <h2 className="text-2xl font-heading font-semibold text-stone-900 dark:text-stone-50 mb-4">Problem</h2>
          {descriptionHtml ? (
            ...
            <p className="text-stone-500 dark:text-stone-400">
              ...
              <a href={leetcodeProblemUrl(problem.slug)} target="_blank" rel="noopener noreferrer" className="underline hover:text-stone-900 dark:hover:text-stone-50">
                LeetCode
              </a>
              .
            </p>
        ...
        <section className="lg:sticky lg:top-28">
          <h2 className="text-2xl font-heading font-semibold text-stone-900 dark:text-stone-50 mb-4">Solution</h2>
            ...
            <div className="flex items-center gap-2 px-3 py-2 border-b border-stone-800">
              <div className="flex items-center gap-1 min-w-0 overflow-x-auto">
                {problem.solutions.length > 1 ? (
                  ...
                  <span className="px-3 py-1 text-xs font-medium text-stone-400 dark:text-stone-500">{LANGUAGE_LABELS[solution.language]}</span>
                )}
              </div>
              <button
                type="button"
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2 mt-4 text-sm">
            <a
              ...
              className="inline-flex items-center gap-1.5 text-stone-500 dark:text-stone-400 hover:text-stone-900 dark:hover:text-stone-50 transition-colors"
            >
